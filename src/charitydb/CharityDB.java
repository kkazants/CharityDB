...
public class CharityDB {
    ...
    public static void main(String[] args) throws SQLException, IOException {
        ...
        try {
            ...
            conn.setAutoCommit(false);
            /*
            // inter what user wishes to do
            System.out.println("What would you like to do? \nEnter: 'add donor', 'add company', or 'add donation' without the quotes.");
            String doing = read.readLine();
            // if else = see what needs to be done
            if ("add donor".equals(doing)){
                //add donor
                String[] donor = addDonor();
                
                // have another donor?
                System.out.println("Do you want to add another donor?"
                        + "\nEnter: Y or N");
                String yn = read.readLine();
                if ("Y".equals(yn)){
                    donor = addDonor();
                    ...
                    System.out.println("ERROR: Improper answer, System closed.");
                    stmt.close();
                    conn.close();
                }
                // create connection
                stmt = conn.createStatement();
                String sql;
                sql = "SELECT lastName, firstName FROM donors";
                ResultSet rs = stmt.executeQuery(sql);
            }
            else if ("add company".equals(doing)){
                // add company
                ...
                // Unknown command or misspelled
                System.out.println("ERROR: Unknown command or command misspelled");
                System.out.println(doing);
            }
*/
            /* Leave for sample
            ...
            stmt = conn.createStatement();
            String sql;
            sql = "SELECT DISTINCT donorID, firstName, lastName, address, city, state, zip FROM donors "
                    + "WHERE NOT EXISTS (SELECT * FROM donations "
                    + "WHERE donations.donorID = donors.donorID) "
                    + "ORDER BY donorID, lastName, firstName";
            ResultSet rs = stmt.executeQuery(sql);
